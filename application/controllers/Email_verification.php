...
class Email_verification extends CI_Controller {
	...
	function verify($token){
		...
		if($data == '0') {
			$msg['type'] = "failed";
			$msg['msg'] = "Email verification failed. Please click resend activation email at menu [Register].";
		...
		if($verify_status == 'Y') {
			$msg['type'] = "failed";
			$msg['msg'] = "This email address has already been verified. You can login with this email address.";
			$this->session->set_flashdata('fc_alert', $msg);
			redirect('/');
			return;
		}
		...
		$msg['type'] = "done";
		$msg['msg'] = "Email verification success. You can login with this email address.";
		$this->session->set_flashdata('fc_alert', $msg);
		redirect('/');
	}
