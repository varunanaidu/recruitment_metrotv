<?php if ( !defined('BASEPATH') )exit('No direct script access allowed');?>
												...
												<?php
													if ( isset($data_vacant) and $data_vacant != '0' ){
														foreach($data_vacant as $row)
															echo "<option value=\"{$row->vacant_id}\">{$row->vacant_title} ({$row->a} s/d {$row->b}) - {$row->total_candidat} Applicants (".ucwords(strtolower($row->c)).")</option>";
													}
												?>
											</select>
										</div>
	...
	<script src="<?php echo base_url()?>media/backend/js/pages/applicant/applicant.js?v=1.0.2"></script>
    <!-- Demo Js -->
