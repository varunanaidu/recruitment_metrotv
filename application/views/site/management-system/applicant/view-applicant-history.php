<?php if ( !defined('BASEPATH') )exit('No direct script access allowed');?>
												...
												<?php
													if ( isset($data_vacant) and $data_vacant != '0' ){
														foreach($data_vacant as $row)
															echo "<option value=\"{$row->vacant_id}\">{$row->vacant_title} ({$row->a} s/d {$row->b}) - {$row->total_candidat} Applicants</option>";
													}
												?>
											</select>
										</div>
								...
								<table class="table table-bordered table-striped table-hover js-basic-example dataTable">
									<thead>
										<tr>
											...
											<th class="text-center">JK/Age/Status</th>
	...
	<script src="<?php echo base_url()?>media/backend/js/pages/applicant/applicant-history.js"></script>	
    <!-- Demo Js -->
