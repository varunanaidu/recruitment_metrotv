$(function(){
	...
	//CKEditor
    CKEDITOR.replace('testimoni_content');
    CKEDITOR.config.height = 100;
	
	$("#btn-add").on("click", function(){
		$("#type").val ( "add" );
		$("#content-img").hide();
		$("#testimoni_file").prop("required", true);
	...
	$(".js-basic-example").on("click", ".btn-edit", function(){
		...
		$.ajax({
			url : base_url + "testimoni/ajax_finder",
			type : "POST",
			dataType : "JSON",
			data : data,
			success : function(data){
				if ( data.type == 'done' ){
					...
					$("#id").val ( data.msg[0].a );
					$("#type").val ( "edit" );
					$("#name").val ( data.msg[0].b );
					$("#batch").val ( data.msg[0].c );
					CKEDITOR.instances['testimoni_content'].setData( data.msg[0].d );
					$("#img-data").attr("src", base_url + "../assets/testimoni/" + data.msg[0].e );
					$("#content-img").show();
					$("#testimoni_file").prop("required", false);
					$("#defaultModal").modal ( "show" );
				}
				else{
					swal({
	...
	$("#default-form").ajaxForm({
		...
		url : base_url + "testimoni/ajax_validation",
		dataType : "JSON",
	...
	$(".js-basic-example").on("click", ".btn-remove", function(){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url : base_url + "testimoni/ajax_remove",
					type : "POST",
					data : data,
					dataType : "JSON",
					success : function(data){
