$(function(){
	...
	$(".js-basic-example").on("click", ".btn-edit", function(){
		...
		$.ajax({
			url : base_url + "temp_venue/ajax_finder",
			type : "POST",
			dataType : "JSON",
			data : data,
			success : function(data){
				if ( data.type == 'done' ){
					$("#type").val ( "edit" );
					$("#id").val ( data.msg[0].a );
					$("#temp_name").val ( data.msg[0].b );
					$("#temp_content").val ( data.msg[0].c );
					$("#defaultModal").modal ( "show" );
				}
				else{
					swal({
	...
	$("#default-form").ajaxForm({
		url : base_url + "temp_venue/ajax_validation",
		dataType : "JSON",
	...
	$(".js-basic-example").on("click", ".btn-remove", function(){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url : base_url + "temp_venue/ajax_remove",
					type : "POST",
					data : data,
					dataType : "JSON",
					success : function(data){
